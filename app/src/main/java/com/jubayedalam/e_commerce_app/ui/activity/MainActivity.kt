...
import androidx.core.content.ContextCompat
import androidx.fragment.app.Fragment
import com.devsstore.devsstore.R
import com.devsstore.devsstore.databinding.ActivityMainBinding
import com.devsstore.devsstore.ui.fragment.profile.ProfilesFragment
import com.devsstore.devsstore.ui.fragment.shopping.cart.CartFragment
import com.devsstore.devsstore.ui.fragment.shopping.home.HomeFragment
import com.devsstore.devsstore.ui.fragment.shopping.product.SofaSetFragment
import com.devsstore.devsstore.ui.fragment.shopping.search.SearchFragment
import com.luseen.spacenavigation.SpaceItem
import com.luseen.spacenavigation.SpaceNavigationView
import com.luseen.spacenavigation.SpaceOnClickListener
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        spaceNavigationView.setSpaceOnClickListener(object : SpaceOnClickListener {
            override fun onCentreButtonClick() {
                // Launch a coroutine for fragment transaction
            ...
            override fun onItemClick(itemIndex: Int, itemName: String?) {
                ...
                CoroutineScope(Dispatchers.Main).launch {
                    when (itemIndex) {
                        0 -> replaceFragment(HomeFragment())
                        1 -> replaceFragment(SofaSetFragment())
                        2 -> replaceFragment(SearchFragment())
                        3 -> replaceFragment(ProfilesFragment())
                    }
                }
            }
        ...
        if (intent.hasExtra("navigate_to_fragment")) {
            val fragmentTag = intent.getStringExtra("navigate_to_fragment")
            if (fragmentTag == "sofaset_fragment") {
                // Launch a coroutine to handle fragment transaction
                CoroutineScope(Dispatchers.Main).launch {
                    // Navigate back to SofaSetFragment
                    supportFragmentManager.popBackStack("sofaset_fragment", 0)
                    Log.d("MainActivity", "Fragment tag: $fragmentTag")
                    ...
                    // Display SofaSetFragment
                    val transaction = supportFragmentManager.beginTransaction()
                    transaction.replace(R.id.fragmentContainerView2, SofaSetFragment.newInstance(), "sofaset_fragment")
                    transaction.addToBackStack("sofaset_fragment")
                    spaceNavigationView.changeCurrentItem(1) // Update bottom nav view to indicate SofaSetFragment
                    transaction.commit()
                }
    ...
    // Function to replace fragments
    private suspend fun replaceFragment(fragment: Fragment) {
